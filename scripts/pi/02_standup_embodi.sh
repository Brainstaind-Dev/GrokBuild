...
USER_EMBODI_ENV="${XDG_CONFIG_HOME:-$HOME/.config}/embodi/env"
cat > "$ENV_FILE" <<EOF
...
# Secrets (XAI_API_KEY, etc.) — never committed; create with chmod 600
if [ -f $(printf '%q' "$USER_EMBODI_ENV") ]; then
  # shellcheck disable=SC1091
  set -a
  source $(printf '%q' "$USER_EMBODI_ENV")
  set +a
fi
EOF
ok "Wrote shell helper: $ENV_FILE"
info "In new terminals:  source $ENV_FILE"
if [[ -f "$USER_EMBODI_ENV" ]]; then
  ok "Will also source secrets from $USER_EMBODI_ENV"
else
  warn "No $USER_EMBODI_ENV yet — create it for XAI_API_KEY (chmod 600)"
  info "Example:  mkdir -p ~/.config/embodi && echo 'export XAI_API_KEY=xai-...' > ~/.config/embodi/env && chmod 600 ~/.config/embodi/env"
fi
