...
```bash
mkdir -p ~/.config/embodi
nano ~/.config/embodi/env
# one line:
#   export XAI_API_KEY=xai-your-secret-here
chmod 600 ~/.config/embodi/env
```

- `scripts/pi/env.pi.sh` (regenerated by standup) **sources** `~/.config/embodi/env` when present.
- HIAgent also loads that file in Python via `HIAgent.env_bootstrap` (same path on Windows: `%USERPROFILE%\.config\embodi\env`).
- Existing process/user environment variables are **not** overwritten by the file.
- Never commit the key. Repo `.env` is optional and gitignored.

Verify without printing the secret:

```bash
source ~/.config/embodi/env
python -c "import os; k=os.environ.get('XAI_API_KEY',''); print('OK' if k.startswith('xai-') else 'MISSING', len(k))"
```
