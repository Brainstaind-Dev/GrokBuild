# Vault — Locked decisions

Append with date. Do not silently reverse without user confirmation.

| Date | Decision |
|------|----------|
| 2026-07 | Cortex does **not** re-fuse; Coherence owns fusion |
| 2026-07 | One VirtualSensorSimulator per live process |
| 2026-07/08 | Pi deploy = **git only** |
| 2026-08 | Scaffold: mics in **featured ears**, Pi in **torso**, latch-first joins |
| 2026-08 | UE avatar = separate app (not shell v1) |
| 2026-08 | Blender = **Lab MCP** (`I:\Tools\blender_mcp`), not ahujasid |
| 2026-08 | Pin `mcp` **1.x** for FastMCP / blmcp |
| 2026-08 | User **opted out** of “Help improve Grok” for this project (privacy) |
| 2026-08-10 | Context multi-vault at `Context/` is repo source of truth for session recovery |
| 2026-08-11 | **Print durability:** fewer fragile points = better prints. Avoid skinny cantilevers, hairline walls, isolated bosses, lacey grills until needed; prefer chunky continuous shells, filleted junctions, integrated features |
| 2026-08-11 | **Print prep in Blender:** body in intended print pose (bed Z=0); floating hardware (bosses/tabs) as **separate objects to the side**, not fused/floating on shell. Export `print_bed_*.stl` so Bambu needs no reorientation |
...
- Simulation-first for software features  
- Do not require webcam/mic/RPi for core feature dev  
- Do not commit `.venv/`, `logs/`, `data/`, `cloudflared*.exe`  
- Do not update status report spreadsheets unless asked  
- Scaffold: strength-first printability over decorative cutouts  
